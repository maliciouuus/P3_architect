...
story.append(Paragraph("PHP 8.3 / Laravel 12  ·  React 18  ·  Zustand  ·  SQLite", S("st", fontSize=10, textColor=MGRAY, alignment=TA_CENTER)))
...
transformations = [
    ...
    ("Découplage front / back via API REST",
     "Une couche API REST (routes/api.php) expose 5 endpoints. "
     "Le front React communique exclusivement via ces endpoints avec axios."),
    ("Ajout d'un state management côté front",
     "Zustand gère l'état global partagé entre composants React "
     "(notes, tags, loading, error) selon le pattern Flux."),
]
...
diag_cible = [
    (colors.HexColor("#1e3a5f"), "FRONT-END (React + Zustand)",
     "Composants JSX  ·  Store Zustand  ·  services/api.js"),
    (colors.HexColor("#374151"), "⬇  HTTP REST / JSON  (axios + cookie session)",  ""),
    (colors.HexColor("#1a3a2a"), "routes/api.php",
     "5 endpoints REST  ·  Middleware auth  ·  Préfixe /api"),
...
data = [
    ...
    ["React → API Laravel",    "HTTP REST",     "JSON",       "axios avec withCredentials:true + X-CSRF-TOKEN"],
    ["API → Controller",       "Laravel Router","PHP interne","Middleware auth vérifie la session cookie"],
...
story.append(sp(4))
story.append(body(
    "Tous les endpoints sont préfixés par /api et protégés par le middleware auth. "
    "Format de réponse uniforme : { \"status\": \"success\"|\"error\", \"data\": ..., \"message\": ... }"
))
story.append(sp(6))

# ── Notes endpoints
story.append(h3("Endpoints — Notes"))
story.append(sp(4))
...
endpoints_notes = [
    {
        ...
        "request": "Aucun body. Cookie de session requis.",
...
story.append(h3("Endpoints — Tags"))
story.append(sp(4))
...
added = [
    ["Fichier", "Type", "Rôle"],
    ["app/Services/NoteService.php",                   "Service PHP",      "Logique métier notes — getAllForUser(), create(), delete()"],
    ["app/Services/TagService.php",                    "Service PHP",      "Logique métier tags — getAll(), create()"],
    ["app/Http/Controllers/Api/NoteController.php",    "Controller PHP",   "Gère les requêtes HTTP API notes (index, store, destroy)"],
    ["app/Http/Controllers/Api/TagController.php",     "Controller PHP",   "Gère les requêtes HTTP API tags (index, store)"],
    ["routes/api.php",                                 "Routes Laravel",   "Définit les 5 endpoints REST sous /api"],
    ["resources/js/app.jsx",                           "Entrée React",     "Monte <App /> dans le div#react-app du layout Blade"],
    ["resources/js/components/App.jsx",                "Composant React",  "Composant racine — charge les tags, orchestre les sous-composants"],
    ["resources/js/components/NoteForm.jsx",           "Composant React",  "Formulaire création note — lit le store, appelle addNote()"],
    ["resources/js/components/NoteList.jsx",           "Composant React",  "Liste notes — charge via fetchNotes(), appelle removeNote()"],
    ["resources/js/components/TagForm.jsx",            "Composant React",  "Formulaire création tag — appelle addTag()"],
    ["resources/js/store/useAppStore.js",              "Store Zustand",    "État global (notes, tags, loading, error) + actions"],
    ["resources/js/services/api.js",                   "Service JS",       "Couche d'accès API — getNotes, createNote, deleteNote, getTags, createTag"],
]
...
modified = [
    ...
    ["bootstrap/app.php",
     "Ajout de api: routes/api.php dans withRouting() — Laravel n'expose pas api.php par défaut"],
    ["vite.config.js",
     "Ajout du plugin @vitejs/plugin-react pour compiler le JSX + remplacement de app.js par app.jsx"],
    ["resources/views/dashboard.blade.php",
     "Suppression de <livewire:notes /> et <livewire:tag-form />, ajout de <div id='react-app'>"],
    ["resources/views/partials/head.blade.php",
     "Ajout <meta name='csrf-token'> (lu par api.js) + pointage vers app.jsx"],
    ["package.json",
     "Ajout des dépendances : react, react-dom, zustand, @vitejs/plugin-react"],
]
...
deleted = [
    ...
    ["resources/views/livewire/notes.blade.php",
     "NoteList.jsx + NoteForm.jsx (React)"],
    ["resources/views/livewire/tag-form.blade.php",
     "TagForm.jsx (React)"],
]
...
decisions = [
    ...
    ("Session cookie pour l'authentification API",
     "Plus simple que d'ajouter Laravel Sanctum ou JWT pour ce projet. "
     "Le cookie de session existant est envoyé par axios via withCredentials:true. "
     "Le token CSRF protège les requêtes POST/DELETE contre les attaques CSRF."),
...
story.append(Paragraph(
    "Documentation d'architecture — Projet P3 OpenClassrooms — PHP 8.3 / Laravel 12 / React 18 / Zustand",
    cap_s
))
